...
Under numba that materialises a temporary list on every call and leaks a
steady amount of heap memory. ``lyapunov.mle`` invokes ``utils.dist``
inside an inner loop, so a single MLE estimation over a long series
already triggers a visible leak (hundreds of MB per run).
...
def apply_patches():
    ...
    ``chebyshev_dist`` from the ``utils`` module globals on every call,
    so rebinding the module attributes here is enough to redirect all
    future calls through the non-leaking loop-based implementations.
